...
The P1 proxy
#+begin_src shell
$ ./build/yano-proxy-bin 45000 "192.188.200.214" 45001 "192.188.200.215" 9100 "0.0.0.0" "p1"
#+end_src
** Proxy
The P2 proxy
#+begin_src shell
$ ./build/yano-proxy-bin 45001 nil nil "192.188.200.215" 9100 "0.0.0.0" "p2"
#+end_src
** Frontend
#+begin_src shell
$ ./build/yano-frontend-bin 8000 "http://192.188.200.212:9000"
#+end_src
** Backend
#+begin_src shell
$ ./build/yano-backend-bin 9000 "http://192.188.200.55"
...
** redsocks
Just to adapt the proxy with no detection of sock4 protocol, redsocks solves this issue
...
redsocks {
  ...
  ip = 127.0.0.1;      # ton p1 local
  port = 45000;
  type = socks4;
}
#+end_src
and we start redsocks,
#+begin_src shell
$ redsocks -c /home/spike/redsocks.conf
#+end_src
in my case, as spike login
** iptables
redsocks is located between the client and the proxy. According to the previous configuration, we modify the iptables rules.
#+begin_src emacs-lisp
iptables -t nat -A OUTPUT -p tcp -m owner --uid-owner spike -j RETURN
iptables -t nat -A OUTPUT -p tcp -m owner --uid-owner faye --dport 9000 -j REDIRECT --to-ports 12345
#+end_src
We don't forget the right uid disable in order to stop the "proxy to proxy"
I suppose the proxy and redsocks started with /spike/ login and /faye/ to run the frontend.
If the redsocks user is different, we just have to adjust the rule. for example with the redsocks account.
#+begin_src shell
...
The P1 proxy
#+begin_src shell
$ ./build/yano-proxy-bin 45000 "192.188.200.212" 45001 "192.188.200.215" 9100 "0.0.0.0" "p1"
#+end_src
or
#+begin_src shell
$ ./build/yano-proxy-bin 45000 "192.188.200.212" 9000 "192.188.200.215" 9100 "0.0.0.0" "p1"
#+end_src
** Proxy
The P2 proxy
#+begin_src shell
...
to be independent 
** How
Homemade just to do by myself
** What
Program the less significant object
** Summary
+ Create a proxy to respond as accurately as possible to the patent
+ Be as agnostic as possible
+ Scenario for cataloging and watching videos
+ Explore two areas of use
+ Explain the process step by step
+ Innovate in the encoding function in an alternative way
+ Highlight flexibility
+ Improve for the next step 
