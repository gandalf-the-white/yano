...
(defun parse-args ()
  (let ((args (cdr sb-ext:*posix-argv*)))
    (unless (= (length args) 3)
      (format *error-output*
              "Usage: yano-proxy-bin <listen-port> <target-host> <target-port>~%")
      (sb-ext:exit :code 1))
    (destructuring-bind (listen-port target-host target-port) args
      (values (parse-integer listen-port)
              target-host
              (parse-integer target-port)))))

(defun main ()
  (multiple-value-bind (listen-port target-host target-port)
      (parse-args)
